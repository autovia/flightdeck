...
export default function Dropdown({showRef, onClick, data}) {
  ...
  return (
    <Menu as="div" className="relative inline-block text-left">
      <div>
        <Menu.Button onClick={(e) => e.stopPropagation()} className="inline-flex w-full justify-center gap-x-1.5 rounded-md px-3 py-2 text-sm font-semibold ">
      ...
      >
        <Menu.Items className="absolute right-0 w-52 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
          <div className="py-1">
            <Menu.Item>
            {({ active }) => (
              <a href="#" onClick={(e) => showRef(e)} className={classNames(
            ...
            <Menu.Item>
            {({ active }) => (
              <a href="#" onClick={(e) => onClick(e)} className={classNames(
                    active ? 'bg-gray-100 text-gray-900' : 'text-gray-700',
                    'block px-4 py-2 text-sm'
                  )}>Show details</a>
            )}
